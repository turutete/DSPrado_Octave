...
## Genera_Iarc.m
##
## function arco= Genera_Iarc(V,tinit, Fs)
##
...
##
## Devuelve arco que es una matrix 2 x L, siendo L la longitud del vector
## de entrada.
##
## arco(1,:) = Varc
## arco(2,:) = Iarc
...
function arco = Genera_Iarc(V,tinit, Fs)
 ...
 if (isnumeric(V)==false || isnumeric(tinit)==false || isnumeric(Fs)==false)
  error("Los parámetros de entrada deben ser numéricos");
 endif

 if (isvector(V)==false)
  error("V debe ser un vector 1 X L");
 endif

 if (isscalar(tinit)==false || isscalar(Fs)==false)
  error("tinit y Fs deben ser escalares");
 endif

 L=length(V);

 ninit=floor(tinit*Fs);
 if (ninit>L)
    error("El tiempo de inicio del arco está fuera del rango del vector V");
 endif
...
while (n<=L)
  ...
  if (n>=ninit)
    ...
    if (abs(V(n))> Vd)
      ...
      Vt_corte=sign(V(n))*V0;
      It_corte=(V(n)-Vt_corte)/Rl_arco;
    elseif (abs(V(n))==Vd)
      % Hay 2 puntos de corte
      Vt_corte=sign(V(n))*[Vd V0];
      It_corte=[(V(n)-Vt_corte(1)) (V(n)-Vt_corte(2))]/Rl_arco;
    else
      % Hay 3 puntos de corte
      % El primer punto de corte es la interseción de la recta de carga y
      % la recta (Vd,Id)
      It1=V(n)/(Rc_arco+Rl_arco);
      Vt1=V(n)-It1*Rl_arco;
      ...
      flag_found=0;
      itera=0;
      Vaux=sign(V(n))*(Vd+V0)/2;  % Primer punto de tensión
      Iaux=(V(n)-Vaux)/Rl_arco;   % Primer punto de corriente
      while (flag_found==0)
        Vaux2=(alfa_arco*Rc_arco*Iaux)/(atan(beta_arco*Iaux)*Iaux*Rc_arco+alfa_arco);
        Iaux2=(V(n)-Vaux2)/Rl_arco;
